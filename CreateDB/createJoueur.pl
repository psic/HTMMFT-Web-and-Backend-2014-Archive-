...
#  my totalEquipe = 7700;
  ...
  my $sthJoueur;
	...
	$requeteJoueur = "INSERT INTO joueurs (nom, prenom, age, xp, talent, tactique, technique, physique, vitesse, mental, off, def,drt,ctr, gch, liste_transfert,liste_pret,numero)
	VALUES (?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,0,?); ";
...
#Attaquant
  ...
  if ($ctr < 0){
	...
	#$sthJoueur = $dbh->prepare($requeteJoueur);
	$sthJoueur->execute($nom, $prenom,$age,$xp,$talent,$tactique,$technique,$physique,$vitesse,$mental,$off,$def,$dr,$ctr,$gch,$numero);
...
#Attaquant
  ...
  if ($ctr < 0){
	...
	#VALUES ('$nom', '$prenom',$age,$xp,$talent,$tactique,$technique,$physique,$vitesse,$mental,$off,$def,$dr,$ctr,$gch,0,0); ";
	#$sthJoueur = $dbh->prepare($requeteJoueur);
	$sthJoueur->execute($nom, $prenom,$age,$xp,$talent,$tactique,$technique,$physique,$vitesse,$mental,$off,$def,$dr,$ctr,$gch,0);
